package com.murong.common.shared;
...
public class MagiskExtend {
    ...
    public static String MAGISK_PATH = "/sbin/.core/img/murong_systemless/";
    ...
    private static String MAGISK_MODULE_NAME = "murong_systemless";
    ...
    public static boolean createFileReplaceModule(String orginPath, String newfile, String moduleName, String moduleTitle) {
        if (MagiskExtend.magiskSupported() && RootFile.INSTANCE.itemExists(newfile)) {
            String moduleProp = "id=" + moduleName + "\\n" +
                    ...
                    "versionCode=1\\n" +
                    "author=murong\\n" +
                    "description=用于将第三方应用转换成系统应用的模块，由慕容调度工具箱创建并添加\n" +
                    "minMagisk=17000\\n";
    ...
    public static void magiskModuleInstall(Context context) {
        String moduleProp = "id=murong_systemless\n" +
                "name=慕容调度工具箱的附加模块\n" +
                "version=v1\n" +
                "versionCode=1\n" +
                "author=murong\n" +
                "description=慕容调度工具箱提供的Magisk拓展模块，从而在不修改系统文件的情况下，更改一些参数\n" +
                "minMagisk=17000\n";
        ...
        if (MagiskVersion < 19) {
            ...
            MAGISK_PATH = "/data/adb/magisk_merge_tmnt/murong_systemless/";
            writeModuleFile("", "auto_mount", context);
        } else {
            writeModuleFile("", "update", context);
        }
